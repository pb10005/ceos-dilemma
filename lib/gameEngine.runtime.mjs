...
const DEFAULT_STRATEGY = {
  id: 'balanced', name: 'バランス型', demandBaseMultiplier: 1, adEffectMultiplier: 1,
  unitCostMultiplier: 1, payrollMultiplier: 1, valuationMultiplier: 1
}

export const processTurn = (state, decisions, event, strategy = DEFAULT_STRATEGY) => {
  ...
  return {
    nextState: {
      ...
      valuation: Math.max(0, state.valuation + netIncome * 8 * strategy.valuationMultiplier), employees: Math.max(1, state.employees + decisions.hireCount),
    ...
    operationalMetrics: {
      demand, adDemandEffect, adSpend: decisions.adSpend, unitsSold,
      beginningInventory: state.inventory, productionUnits: decisions.productionUnits, endingInventoryUnits,
      employeesBefore: state.employees, hireCount: decisions.hireCount,
      employeesAfter: Math.max(1, state.employees + decisions.hireCount),
      grossMarginPct: revenue > 0 ? (grossProfit / revenue) * 100 : 0,
      repeatDemand, qualityDemandBoost,
      brandPowerBefore: state.brandPower, brandGain,
      productQualityBefore: state.productQuality, qualityGain,
      customerBaseBefore: state.customerBase, customerGain,
