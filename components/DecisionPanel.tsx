...
const NumberInput = ({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) => (
  <label className="flex flex-col gap-1 text-sm">
    <span>{label}</span>
    <input className="rounded border border-slate-700 bg-slate-800 p-2" type="number" value={value} onChange={(e: any) => onChange(Number(e.target.value))} />
  </label>
)
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4">
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <NumberInput label="採用人数" value={decisions.hireCount} onChange={(v) => onChange({ ...decisions, hireCount: v })} />
      ...
      <label className="mt-3 flex items-center gap-2 text-sm">
        <input type="checkbox" checked={decisions.raiseEquity} onChange={(e: any) => onChange({ ...decisions, raiseEquity: e.target.checked })} />
        エクイティ調達を実行
      </label>
      <button className="mt-4 rounded bg-emerald-400 px-4 py-2 font-semibold text-slate-950" onClick={onNextTurn}>次の四半期へ</button>
    </div>
  )
}
